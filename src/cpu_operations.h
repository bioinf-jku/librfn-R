...
template<typename T>
T init_invalid(void) { return (typeid(T) == typeid(spmat_t) ? (T) -1 : (T) 0); }
...
void gemm(const char *transa, const char *transb, const int m,
                 const int n, const int k, const float alpha,
                 const spmat_t a, const int lda, const float *b,
                 const int ldb, const float beta, float *c,
                 const int ldc) const 
{
...
void gemm(const char *transa, const char *transb, const int m,
                 const int n, const int k, const float alpha,
                 const float *a, const int lda, const spmat_t b,
                 const int ldb, const float beta, float *c,
                 const int ldc) const 
{
...
float *memcpy_matrix( float *dest, float *src, int nrows_to_copy, int src_ncol, 
...
spmat_t memcpy_matrix( spmat_t &dest, spmat_t src, int nrows_to_copy, int src_ncol, 
   int first_row = 0) const 
{
   free(dest);
...
void calculate_column_variance(spmat_t X, const unsigned nrows, 
   const unsigned ncols, float* variances)
{
...
static void printMatrixCM(const spmat_t a, int n, int m, const char *fmt) { NIST_SPBLAS::print(a); }

static void printMatrixRM(const float* a, int n, int m, const char* fmt);
static void printMatrixRM(const spmat_t a, int n, int m, const char *fmt) { NIST_SPBLAS::print(a); }
};
